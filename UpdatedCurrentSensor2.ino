...
float readCurrentINA219() {
  float current = ina219.getCurrent_mA() / 1000.0; // A (can be negative)

  // Clamp small values around zero (noise)
...
// ---------- Draw Centered Current Bar ----------
// ---------- Draw Centered Current Bar ----------
void drawCurrentBar(float current) {
  int centerX = 64;   // OLED center (128px wide)
  int barMax = 50;    // Maximum bar length in pixels
  int barY = 50;      // Y-position of the bar

  // Scale current to bar length based on ±3A
  int barLength = map(abs(current * 1000), 0, 3000, 0, barMax);

  // Clear previous bar area
  u8g2.setDrawColor(0);
  u8g2.drawBox(centerX - barMax, barY, barMax * 2, 10);
  u8g2.setDrawColor(1);

  // Draw boundary frame
  u8g2.drawFrame(centerX - barMax, barY, barMax * 2, 10);

  // Draw bar: positive → right, negative → left
  if (current > 0) {
    u8g2.drawBox(centerX, barY, barLength, 10);
  } else if (current < 0) {
    u8g2.drawBox(centerX - barLength, barY, barLength, 10);
  }

  // Draw center line (zero axis)
  u8g2.drawLine(centerX, barY - 2, centerX, barY + 12);
}
...
void loop() {
  ...
  // Current
// Current
u8g2.setCursor(10, 48);  // Slightly lower than before
u8g2.print("I: ");
